// @ts-ignore
const mysql = require('mysql2/promise');

async function testMonitorsEndpoint() {
  const dbConfig = {
    host: '127.0.0.1',
    user: 'majesticflame',
    password: 'Tu_Passw0rd!23',
    database: 'ccio',
    port: 3306
  };

  let connection;
  try {
    connection = await mysql.createConnection(dbConfig);

    // Simular la consulta que hace el endpoint de monitores
    const [rows] = await connection.query(
      'SELECT mid, ke, name, type, protocol, host, port, path, mode, ext, fps, width, height, details FROM Monitors WHERE ke = ? LIMIT 10',
      ['63aaObjyC9']
    );

    console.log('Monitores encontrados:');
    console.table(rows);

    if (rows.length > 0) {
      console.log('\nGenerando URLs de streams para el primer monitor:');
      const monitor = rows[0];

      // Configuración de Shinobi
      const SHINOBI_HOST = 'http://localhost:8080';
      const SHINOBI_API_KEY = 'Tu_Passw0rd!23';

      // Construir las URLs de los streams
      const basePath = `${SHINOBI_HOST}/${monitor.ke}/${monitor.mid}`;
      const authParam = `?auth=${SHINOBI_API_KEY}`;

      const streams = {
        hls: `${basePath}/hls/monitor.m3u8${authParam}`,
        mjpeg: `${basePath}/mjpeg.jpg${authParam}`,
        flv: `${basePath}/flv${authParam}`
      };

      console.log('URLs de streams generadas:');
      console.log('- HLS:', streams.hls);
      console.log('- MJPEG:', streams.mjpeg);
      console.log('- FLV:', streams.flv);
    }

  } catch (error) {
    console.error('Error:', error.message);
  } finally {
    if (connection) await connection.end();
  }
}

testMonitorsEndpoint();
